...
USAGE: slp_to_mp4.py REPLAY_FILE [OUT_FILE]
...
def combine(conf, already_combined):
    created_files = []
    for subdir, dirs, files in os.walk(OUT_DIR, topdown=False):
        ...
        with open(os.path.join(subdir, 'concat_file.txt'), 'w+') as concat_file:
            ...
            for file in files:
                try:
                    if file.endswith('.mp4') and os.path.join(subdir, file) not in already_combined:
        ...
        output_fpath = os.path.join(OUT_DIR, basedir) + '.mp4'

        # If there is 1 or more mp4 file and not overwriting an existing file
        if file_count > 0 and not os.path.exists(output_fpath):
            ffmpeg_runner = FfmpegRunner(conf.ffmpeg)
            ffmpeg_runner.combine(os.path.join(subdir, 'concat_file.txt'), output_fpath)
            created_files.append(output_fpath)
            already_combined.append(output_fpath)
...
def record_folder_slp(slp_folder, conf, final_outfile):
    in_files = []
    out_files = []
    combined_files = []
    out_file = None
    ...
    if conf.combine:
        combined = combine(conf, combined_files)
        if len(combined) > 0:
            out_file = combined[0]  # should really only have 1 item~

    if final_outfile is not None and out_file is not None:
        shutil.copy2(out_file, final_outfile)
        shutil.rmtree(OUT_DIR)  # just do a full cleanup
...
def main(args=None):
    ...
    slp_file = os.path.abspath(args[1])
    clean()
    ...
    if len(args) > 2:
        if args[2].endswith('.mp4'):
            outdir, outfile_name = os.path.split(args[2])
        else:
            outdir = args[2]
    ...
    if os.path.isdir(slp_file):
        conf = Config()
        record_folder_slp(slp_file, conf, outfile)
    else:
